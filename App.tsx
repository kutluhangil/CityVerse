...
function App() {
  ...
  const [demolishedTiles, setDemolishedTiles] = useState<
    {
      x: number;
      y: number;
      id: number;
  ...
  const fetchNewGoal = useCallback(async () => {
    if (isGeneratingGoal || !aiEnabledRef.current || !isGeminiConfigured)
      return;
    ...
    if (newGoal) {
      ...
      if (aiEnabledRef.current && isGeminiConfigured)
        setTimeout(fetchNewGoal, 5000);
    }
  ...
  const fetchNews = useCallback(async () => {
    ...
    if (!aiEnabledRef.current || !isGeminiConfigured || Math.random() > 0.15)
      return;
  ...
  useEffect(() => {
    ...
    if (aiEnabled && isGeminiConfigured) {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      gridRef.current.flat().forEach((tile) => {
        if (tile.buildingType !== BuildingType.None) {
          ...
          buildingCounts[tile.buildingType] =
            (buildingCounts[tile.buildingType] || 0) + levelMultiplier;
        }
      });
      ...
      const happinessMultiplier = Math.max(
        0.2,
        statsRef.current.happiness / 60,
      );
      ...
      setStats((prev) => {
        ...
        if (resCount === 0 && prev.population > 0)
          newPop = Math.max(0, prev.population - 5); // people leave if no homes
        ...
        const parksPerCapitaBonus =
          prev.population > 0
            ? ((parkCount + playCount * 3 + fountCount * 5) /
                (prev.population / 100)) *
              10
            : 0;
        ...
        if (Math.random() > 0.8) {
          const choices: ("sunny" | "rainy" | "snowy")[] = [
            "sunny",
            "sunny",
            "rainy",
            "snowy",
          ...
          if (nextWeather !== newWeather) {
            ...
            if (newWeather === "rainy" && Math.random() > 0.3) {
              addNewsItem({
                ...
                text: "Citizen: 'Make sure to bring an umbrella today!'",
                type: "neutral",
              });
            } else if (newWeather === "snowy" && Math.random() > 0.3) {
              addNewsItem({
                id: Date.now().toString() + Math.random(),
                text: "Citizen: 'The snow is beautiful, but the roads are slippery.'",
                type: "neutral",
              });
            } else if (newWeather === "sunny" && prev.weather !== "sunny") {
              addNewsItem({
                id: Date.now().toString() + Math.random(),
                text: "Citizen: 'Finally, some clear skies!'",
                type: "positive",
              });
            }
          }
              ...
              for (let dy = -2; dy <= 2; dy++) {
                for (let dx = -2; dx <= 2; dx++) {
                  ...
                  if (ny >= 0 && ny < GRID_SIZE && nx >= 0 && nx < GRID_SIZE) {
                    if (
                      gridRef.current[ny][nx].buildingType ===
        ...
        const pollutionImpact =
          resCount > 0 ? localizedPollutionPenalty / resCount : 0;

        let targetHappiness =
          50 +
          varietyBonus +
          Math.min(30, parksPerCapitaBonus) +
          comCount * 2 -
          pollutionImpact +
          weatherMod;
        ...
        if (Math.abs(targetHappiness - currentHap) > 1) {
          newHappiness = Math.round(
            currentHap + (targetHappiness - currentHap) * 0.1,
          );
          ...
          if (targetHappiness < currentHap - 10 && Math.random() > 0.5) {
            addNewsItem({
              id: Date.now().toString() + Math.random(),
              text: "Citizen: 'The city feels so bleak lately...'",
              type: "negative",
            });
          } else if (targetHappiness > currentHap + 10 && Math.random() > 0.5) {
            addNewsItem({
              id: Date.now().toString() + Math.random(),
              text: "Citizen: 'Things are really looking up around here!'",
              type: "positive",
            });
          }
        }
        ...
        if (aiEnabledRef.current && goal && !goal.completed) {
          ...
          if (
            ...
            isMet = true;
          if (
            goal.targetType === "happiness" &&
            newStats.happiness >= goal.targetValue
          )
            isMet = true;
          if (goal.targetType === "building_count" && goal.buildingType) {
            if ((buildingCounts[goal.buildingType] || 0) >= goal.targetValue)
              isMet = true;
          }
        ...
        if (currentTile.buildingType !== BuildingType.None) {
          ...
          if (currentStats.money >= demolishCost) {
            ...
            const refund =
              ...
              (currentTile.level && currentTile.level > 1
                ? (currentTile.level - 1) * 25
                : 0);
            newGrid[y][x] = {
              ...currentTile,
            ...
            // Trigger animation
            const id = Date.now();
            setDemolishedTiles((prev) => [
              ...prev,
              {
                x,
                y,
                id,
                type: currentTile.buildingType,
                level: currentTile.level,
                color: BUILDINGS[currentTile.buildingType].color,
              },
            ]);
            setTimeout(() => {
            ...
            if (refund > 0) {
              addNewsItem({
                id: Date.now().toString(),
                text: `Refund of $${refund} received for demolishing upgraded building.`,
                type: "positive",
              });
            }
          } else {
      ...
      if (currentTile.buildingType === BuildingType.None) {
        ...
        if (tool !== BuildingType.Road && tool !== BuildingType.Park) {
          if (!checkRadius(BuildingType.Road, 1)) {
            addNewsItem({
              id: Date.now().toString() + Math.random(),
              text: "Buildings must be placed next to a road.",
              type: "negative",
            });
            return;
          }
        ...
        if (
          tool === BuildingType.Industrial &&
          checkRadius(BuildingType.Residential, 3)
        ) {
          addNewsItem({
            id: Date.now().toString() + Math.random(),
            text: "Zoning error: Industrial cannot be within 3 blocks of Residential.",
            type: "neutral",
          });
          return;
        }
        ...
        if (
          tool === BuildingType.Residential &&
          checkRadius(BuildingType.Industrial, 3)
        ) {
          addNewsItem({
            id: Date.now().toString() + Math.random(),
            text: "Zoning error: Residential cannot be within 3 blocks of Industrial.",
            type: "neutral",
          });
          return;
        }
        ...
        if (currentStats.money >= buildingConfig.cost) {
          ...
          if (
            tool === BuildingType.Park ||
            tool === BuildingType.ParkFountain ||
            tool === BuildingType.ParkPlayground
          ) {
            addNewsItem({
              id: Date.now().toString() + Math.random(),
              text: `Citizen: "Finally, a new place to relax! Love the new ${buildingConfig.name}."`,
              type: "positive",
            });
          } else if (tool === BuildingType.Industrial) {
            addNewsItem({
              id: Date.now().toString() + Math.random(),
              text: `Citizen: "Great for the economy, but I hope the smog isn't too bad..."`,
              type: "neutral",
            });
          }
        } else {
  ...
  const handleStart = (enabled: boolean) => {
    setAiEnabled(enabled && isGeminiConfigured);
    setGameStarted(true);
  };
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden selection:bg-transparent selection:text-transparent bg-sky-900">
      ...
      {!gameStarted && (
        <StartScreen onStart={handleStart} aiAvailable={isGeminiConfigured} />
      )}
      ...
      {gameStarted && (
        <>
          ...
          {inspectedTile && (
            ...
            >
              <div
                className="bg-gray-900 border border-gray-600 rounded-xl p-6 w-full max-w-sm text-white shadow-2xl"
              ...
              >
                ...
                {(() => {
                  ...
                  const nextName =
                    currentLevel < maxLevel ? levelNames[currentLevel] : "Max";

                  const handleUpgrade = () => {
                    if (currentLevel >= maxLevel) return;
                    if (stats.money >= upgradeCost) {
                      const newGrid = grid.map((row) => [...row]);
                      newGrid[inspectedTile.y][inspectedTile.x] = {
                        ...tile,
                      ...
                      addNewsItem({
                        id: Date.now().toString(),
                        text: `Citizen: "Wow, the new ${bConfig.name} upgrade to ${nextName} looks amazing!"`,
                        type: "positive",
                      });
                      ...
                      addNewsItem({
                        id: Date.now().toString(),
                        text: "Insufficient funds to upgrade building.",
                        type: "negative",
                      });
                    }
                  };
                    ...
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex gap-2">
                          {[1, 2, 3].map((l) => (
                            <div
                              key={l}
                              className={`w-1/3 h-2 rounded ${l <= currentLevel ? "bg-purple-500" : "bg-gray-700"}`}
                            />
                          ))}
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 bg-gray-800 rounded p-3 text-sm">
                        <div className="flex flex-col">
                          ...
                          >
                            {bConfig.incomeGen * currentLevel > 0 ? "+" : ""}
                            {bConfig.incomeGen * currentLevel} / day
                          </span>
                        </div>
                        <div className="flex flex-col">
                          ...
                          >
                            {bConfig.popGen * currentLevel > 0 ? "+" : ""}
                            {bConfig.popGen * currentLevel} / max
                          </span>
                        </div>
                      </div>
                      {currentLevel < maxLevel ? (
                        ...
                        <button
                          onClick={() => {
                            ...
                            const demolishCost =
                              BUILDINGS[BuildingType.Demolish].cost;
                            const refund =
                              currentTile.level && currentTile.level > 1
                                ? (currentTile.level - 1) * 20
                                : 0;
                            ...
                            if (stats.money >= demolishCost) {
                              ...
                              setDemolishedTiles((prev) => [
                                ...prev,
                                {
                                  ...
                                  level: currentTile.level,
                                  color:
                                    BUILDINGS[currentTile.buildingType].color,
                                },
                              ]);
                              setTimeout(() => {
                                setDemolishedTiles((prev) =>
                                  prev.filter((dt) => dt.id !== id),
                                );
                              }, 2000);
                              ...
                              if (refund > 0) {
                                addNewsItem({
                                  id: Date.now().toString(),
                                  text: `Redeveloped a max level building for a $${refund} refund!`,
                                  type: "positive",
                                });
                              }
                            }
                          ...
                          disabled={
                            stats.money < BUILDINGS[BuildingType.Demolish].cost
                          }
